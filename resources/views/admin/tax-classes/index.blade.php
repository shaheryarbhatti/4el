...
.stat-card--default .stat-card__icon { background:linear-gradient(135deg,#3b82f6,#60a5fa); color:#fff; }
.stat-card--default .stat-card__bar  { background:linear-gradient(90deg,#3b82f6,#60a5fa); }
.stat-card--default .stat-card__footer i { color:#3b82f6; }
.stat-card--rate    .stat-card__icon { background:linear-gradient(135deg,#f59e0b,#fbbf24); color:#fff; }
.stat-card--rate    .stat-card__bar  { background:linear-gradient(90deg,#f59e0b,#fbbf24); }
.stat-card--rate    .stat-card__footer i { color:#f59e0b; }
...
/* ── DataTable ── */
#taxTable { width:100% !important; border-collapse:separate; border-spacing:0; }
#taxTable thead th { background:#f8fafc; color:#475569; font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.07em; padding:13px 16px; border-bottom:2px solid #e2e8f0; white-space:nowrap; }
#taxTable tbody td { padding:16px 16px; vertical-align:middle; border-bottom:1px solid #f1f5f9; font-size:13.5px; color:#374151; }
#taxTable tbody tr:last-child td { border-bottom:none; }
#taxTable tbody tr:hover td { background:#fafbff; }
...
/* ── cell widgets ── */
.tax-name { font-weight:700; color:#1e293b; font-size:14px; }
.rate-badge { font-size:22px; font-weight:900; color:#4f46e5; display:inline-flex; align-items:baseline; gap:2px; }
.rate-pct   { font-size:13px; font-weight:700; color:#818cf8; }
.b-default  { background:#dbeafe; color:#1d4ed8; padding:4px 12px; border-radius:20px; font-size:12px; font-weight:700; display:inline-flex; align-items:center; gap:5px; }
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="page-heading">
        <div>
            <h4><i class="bi bi-receipt-cutoff" style="color:#6c63ff;"></i> Tax Classes</h4>
        ...
        <a href="{{ route('admin.tax-classes.create') }}" class="btn-add">
            <span class="btn-add-icon"><i class="bi bi-plus-lg"></i></span>
            <span class="btn-add-label">Add Tax Class</span>
        </a>
    </div>
    ...
    <div class="stat-grid">
        @php
        $cards = [
            ['cls'=>'total',   'icon'=>'bi-receipt-cutoff',   'value'=>$stats['total'],      'label'=>'Total Classes',  'foot'=>'All tax classes'],
            ['cls'=>'active',  'icon'=>'bi-check-circle-fill','value'=>$stats['active'],     'label'=>'Active',         'foot'=>'Available for products'],
            ['cls'=>'default', 'icon'=>'bi-star-fill',        'value'=>$stats['is_default'], 'label'=>'Default',        'foot'=>'Pre-selected on new products'],
            ['cls'=>'rate',    'icon'=>'bi-percent',          'value'=>$stats['avg_rate'].'%','label'=>'Average Rate',  'foot'=>'Across all tax classes'],
        ];
        @endphp
        @foreach($cards as $c)
    ...
    <div class="filter-card">
        <div class="filter-title"><i class="bi bi-funnel-fill" style="color:#6c63ff;"></i> Filter Tax Classes</div>
        <div class="filter-row">
            ...
            <div class="filter-group">
                <label>Default</label>
                <select id="f-default" class="filter-select">
    ...
    <div class="table-card">
        <div class="table-card-head">
            <div class="table-card-title"><i class="bi bi-table"></i> Tax Class List</div>
        ...
        <div class="table-card-body">
            <table id="taxTable" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th>Name</th>
...
$(function () {
    var table = $('#taxTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url : '{{ route('admin.tax-classes.index') }}',
            type: 'GET',
            data: function (d) {
                d.status         = $('#f-status').val();
                d.default_filter = $('#f-default').val();
            },
        ...
        columns: [
            ...
            { data: 'default_html', name: 'is_default', orderable: false, searchable: false },
        ...
        language: {
            search: '', searchPlaceholder: 'Search tax classes…',
            processing: '<span style="color:#6c63ff;">Loading…</span>',
            emptyTable : '<div style="padding:40px 0;text-align:center;color:#94a3b8;"><i class="bi bi-inbox fs-2" style="display:block;margin-bottom:10px;"></i>No tax classes found</div>',
            zeroRecords: '<div style="padding:40px 0;text-align:center;color:#94a3b8;"><i class="bi bi-search fs-2" style="display:block;margin-bottom:10px;"></i>No matching records</div>',
            lengthMenu: 'Show _MENU_ entries',
            info: 'Showing _START_–_END_ of _TOTAL_ tax classes',
            infoEmpty: 'No tax classes available',
