...
.stat-card--logo    .stat-card__icon { background:linear-gradient(135deg,#3b82f6,#60a5fa); color:#fff; }
.stat-card--logo    .stat-card__bar  { background:linear-gradient(90deg,#3b82f6,#60a5fa); }
.stat-card--logo    .stat-card__footer i { color:#3b82f6; }
...
.btn-apply {
    ...
    box-shadow:0 4px 14px rgba(16,185,129,.35); transition:transform .22s, box-shadow .22s;
}
...
.btn-reset {
    ...
    box-shadow:0 4px 14px rgba(244,63,94,.32); transition:transform .22s, box-shadow .22s;
}
...
.btn-add {
    ...
    box-shadow:0 4px 16px rgba(108,99,255,.4); transition:transform .25s, box-shadow .25s, color 0s;
}
...
/* ── DataTable ── */
#brandTable { width:100% !important; border-collapse:separate; border-spacing:0; }
#brandTable thead th { background:#f8fafc; color:#475569; font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.07em; padding:13px 16px; border-bottom:2px solid #e2e8f0; white-space:nowrap; }
#brandTable tbody td { padding:14px 16px; vertical-align:middle; border-bottom:1px solid #f1f5f9; font-size:13.5px; color:#374151; }
#brandTable tbody tr:last-child td { border-bottom:none; }
#brandTable tbody tr:hover td { background:#fafbff; }
...
/* ── cell widgets ── */
.brand-thumb { width:48px; height:48px; border-radius:10px; object-fit:contain; border:1.5px solid #e2e8f0; background:#f8fafc; display:block; margin:0 auto; padding:3px; }
.brand-avatar {
    width:48px; height:48px; border-radius:10px; display:flex; align-items:center; justify-content:center;
    font-size:20px; font-weight:800; color:#fff; margin:0 auto;
    background:hsl(var(--av-hue,260),65%,60%);
}
.brand-name { font-weight:700; color:#1e293b; font-size:14px; }
.brand-slug { color:#94a3b8; font-size:11px; font-family:monospace; }
.brand-url  { font-size:11.5px; color:#6c63ff; text-decoration:none; }
.brand-url:hover { text-decoration:underline; }

.b-feat    { background:#fef3c7; color:#92400e; padding:4px 10px; border-radius:20px; font-size:11.5px; font-weight:600; display:inline-flex; align-items:center; gap:4px; }
...
.prod-count { display:inline-flex; align-items:center; justify-content:center; min-width:32px; height:28px; border-radius:8px; background:#f0f4ff; color:#4f46e5; font-size:13px; font-weight:700; padding:0 8px; }
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="page-heading">
        <div>
            <h4><i class="bi bi-award-fill" style="color:#6c63ff;"></i> Brands</h4>
        ...
        <a href="{{ route('admin.brands.create') }}" class="btn-add">
            ...
            <span class="btn-add-label">Add Brand</span>
        </a>
    </div>
    ...
    <div class="stat-grid">
        @php
        $cards = [
            ['cls'=>'total',  'icon'=>'bi-award-fill',        'value'=>$stats['total'],     'label'=>'Total Brands',    'foot'=>'All brands in system'],
            ['cls'=>'active', 'icon'=>'bi-check-circle-fill', 'value'=>$stats['active'],    'label'=>'Active',          'foot'=>'Visible to shoppers'],
            ['cls'=>'feat',   'icon'=>'bi-star-fill',         'value'=>$stats['featured'],  'label'=>'Featured',        'foot'=>'Shown in home widget'],
            ['cls'=>'logo',   'icon'=>'bi-image-fill',        'value'=>$stats['with_logo'], 'label'=>'With Logo',       'foot'=>'Brands with image uploaded'],
        ];
        @endphp
        @foreach($cards as $c)
    ...
    <div class="filter-card">
        <div class="filter-title"><i class="bi bi-funnel-fill" style="color:#6c63ff;"></i> Filter Brands</div>
    ...
    <div class="table-card">
        <div class="table-card-head">
            <div class="table-card-title"><i class="bi bi-table"></i> Brand List</div>
...
$(function () {
    var table = $('#brandTable').DataTable({
        ...
        columns: [
            { data: 'logo_html',     name: 'logo',         orderable: false, searchable: false, className: 'text-center' },
            { data: 'name_html',     name: 'name',         orderable: true  },
            { data: 'featured_html', name: 'is_featured',  orderable: false, searchable: false },
            { data: 'status_html',   name: 'is_active',    orderable: false, searchable: false },
            { data: 'products_html', name: 'products_count', orderable: false, searchable: false, className: 'text-center' },
        ...
        language: {
            ...
            emptyTable        : '<div style="padding:40px 0;text-align:center;color:#94a3b8;"><i class="bi bi-inbox fs-2" style="display:block;margin-bottom:10px;"></i>No brands found</div>',
            zeroRecords       : '<div style="padding:40px 0;text-align:center;color:#94a3b8;"><i class="bi bi-search fs-2" style="display:block;margin-bottom:10px;"></i>No matching brands</div>',
